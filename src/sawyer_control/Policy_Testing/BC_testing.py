...
def test_diffusion_policy():
    ...
    # Path to your BC
    ep_id = 1000
    model_path = f"/home/tung/workspace/rlhf_bench/iql-pytorch-sawyer/checkpoints/{task_name}/behavioral-cloning/bc_actor_ep{ep_id}.pth"
    output_dir = f"/home/tung/workspace/rlhf_bench/iql-pytorch-sawyer/output_video/{task_name}/behavioral-cloning/bc_actor_ep_{ep_id}"
    ...
    print(f"--- Loading BC Actor from {model_path} (ID: {ep_id}) ---")
    ...
    for ep in range(num_test_episodes):
        ...
        print(f"\n--- Episode {ep+1} Start ---")
        pub.publish(os.path.join(output_dir,f"episode_{ep+6}.mp4"))
        ...
        for step in range(max_steps):
            ...
            with torch.no_grad():
                action_5d = actor(state_tensor).cpu().data.numpy().flatten()
            print(action_5d)

            # 3. Apply Safety Constraints
            current_gripper_z = state[2]
            z_lower_limit = -0.285
            if current_gripper_z <= z_lower_limit and action_5d[2] < 0.0:
                action_5d[2] = 0.0 # Prevent moving further down
            ...
            cv2.imshow("Sawyer BC Evaluation", obs['rgb_image'][:, :, ::-1])
